...
const MotionClient = require('./external/MotionClient');
...
function parseLC(config, vault) {
    ...
    Object.keys(config.locationConstraints).forEach(location => {
        ...
        if (locationObj.type === 'motion') {
            const motionDaemonEndpoint = config.getMotionDaemonEndpoint(location);
            clients[location] = new MotionClient({
                bucketMatch: locationObj.details.bucketMatch,
                dataStoreName: location,
                endpoint: motionDaemonEndpoint,
            });
            clients[location].clientType = 'motion';
        }
    });
    return clients;
}
